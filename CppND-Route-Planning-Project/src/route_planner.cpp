...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    //std::cout<<RoutePlanner::CalculateHValue(start_node)<<"\n";
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    RouteModel::Node* lowestSumNode;
    sort(open_list.begin(),open_list.end(),Compare);
    lowestSumNode=open_list[0];
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    RouteModel::Node* previous_node;
    path_found.push_back(*current_node);
    distance=distance+current_node->distance(*end_node);

    while(current_node->parent != nullptr) {
        previous_node=current_node->parent;
        distance=distance+current_node->distance(*previous_node);
